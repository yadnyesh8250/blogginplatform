...
export default function DashComments() {
  ...
  useEffect(() => {
    const fetchComments = async () => {
      try {
        const res = await api(`/api/comment/getcomments`);
  ...
  const handleShowMore = async () => {
    ...
    try {
      const res = await api(`/api/comment/getcomments?startIndex=${startIndex}`);
      const data = await res.json();
      if (res.ok) {
  ...
  const handleDeleteComment = async () => {
    setShowModal(false);
    try {
      const res = await api(`/api/comment/deleteComment/${commentIdToDelete}`, { method: 'DELETE' });
      if (res.ok) {
