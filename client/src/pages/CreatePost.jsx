...
export default function CreatePost() {
  ...
  const activeTheme = getTheme(formData.theme);
  ...
  const handleTagsChange = (e) => {
    const tagsArray = e.target.value.split(',').map((tag) => tag.trim()).filter(t => t !== '');
    ...
    >
      ...
      >
        <div className='flex items-center gap-3'>
          ...
          <span className='text-[10px] px-2 py-0.5 rounded-full font-black uppercase tracking-[0.2em] border' style={{ borderColor: activeTheme.vars['--theme-accent'], color: activeTheme.vars['--theme-accent'] }}>
            {activeTheme.name} Engine
          </span>
        </div>
        <div className='flex gap-3'>
          <button
            type='button'
            className='px-5 py-2 rounded-xl text-xs font-black uppercase tracking-widest bg-slate-100 dark:bg-slate-800 text-slate-500 hover:text-slate-800 dark:hover:text-slate-200 transition-all'
          ...
          <button
            ...
            className='px-6 py-2 rounded-xl text-xs font-black uppercase tracking-widest text-white hover:opacity-90 transition-all shadow-lg active:scale-95'
      ...
      <form id='create-post-form' onSubmit={handleSubmit} className='flex flex-col lg:flex-row gap-0 min-h-screen'>
        {/* ── Main Content Area ── */}
        <div className='flex-1 p-6 lg:p-16 flex flex-col gap-10 max-w-5xl mx-auto w-full' style={{ background: activeTheme.vars['--theme-bg'] }}>
          ...
          {formData.image && (
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className='relative group rounded-[40px] overflow-hidden shadow-2xl'>
              <img src={formData.image} alt='featured' className='w-full h-[500px] object-cover' />
              <button
                type='button'
                onClick={() => setFormData({ ...formData, image: '' })}
                className='absolute top-6 right-6 bg-red-600 text-white px-4 py-2 rounded-2xl text-xs font-black uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-opacity active:scale-95'
              >
                ✕ Remove Image
              </button>
            </motion.div>
        ...
        <div className='w-full lg:w-96 flex-shrink-0 border-l flex flex-col gap-0 overflow-y-auto' style={{ background: activeTheme.vars['--theme-surface'], borderColor: activeTheme.vars['--theme-border'] }}>
          ...
          <SidebarSection title='🚀 Publish Panel' theme={activeTheme}>
             <div className='space-y-4'>
                ...
                >
                  {formData.status === 'published' ? '🚀 Authorize Publish' : '💾 Persist Draft'}
                </button>
             </div>
          </SidebarSection>

          {/* Professional Customization - NEW SECTION */}
          <SidebarSection title='🎨 Advanced Design' theme={activeTheme}>
             <div className='space-y-5'>
                ...
                {/* Font Face Selector */}
                <div className='space-y-2'>
                  <label className='flex items-center gap-2 text-[10px] font-black uppercase tracking-widest opacity-60'><HiPencilAlt /> Topography Style</label>
                    ...
                    {['serif', 'sans-serif', 'mono'].map(font => (
                      <button
                        ...
                        className={`py-2 text-[9px] font-black uppercase tracking-widest border-2 rounded-xl transition-all ${
                          formData.fontFamily === font ? 'border-indigo-500 bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600' : 'border-transparent bg-slate-100 dark:bg-slate-900 text-slate-400'
                        }`}
                      >
                         {font}
                      </button>
          ...
          {/* Metadata & SEO */}
          <SidebarSection title='🔍 Global Visibility' theme={activeTheme}>
             <div className='space-y-4'>
                <div className='flex flex-col gap-1'>
                  <label className='text-[9px] font-black uppercase opacity-50 tracking-widest'>Post Visibility</label>
                  <select
                    className='w-full rounded-xl px-4 py-2.5 text-xs bg-slate-100 dark:bg-slate-900 border-none font-bold'
                    value={formData.visibility}
                    onChange={(e) => setFormData({ ...formData, visibility: e.target.value })}
                  >
                    <option value='public'>🌍 World Public</option>
                    <option value='private'>🔒 Private Circle</option>
                    <option value='password'>🔑 Encrypted</option>
                  </select>
                </div>
          ...
          {/* Categories & Media */}
          <SidebarSection title='🖼️ Assets & Taxonomy' theme={activeTheme}>
             <div className='space-y-6'>
                ...
                {/* Categories */}
                <div className='max-h-40 overflow-y-auto flex flex-col gap-1 px-1'>
                  {categories.map((cat) => (
